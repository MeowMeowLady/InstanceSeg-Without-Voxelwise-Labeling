...
class mask_rcnn_outputs(nn.Module):
    ...
    def __init__(self, dim_in):
        ...
        if cfg.MRCNN.USE_FC_OUTPUT:
            ...
            self.classify = nn.Linear(dim_in, n_classes * cfg.MRCNN.RESOLUTION**3)
        else:
            # Predict mask using Conv
            self.classify = nn.Conv3d(dim_in, n_classes, 1, 1, 0)
            if cfg.MRCNN.UPSAMPLE_RATIO > 1:
                self.upsample = mynn.BilinearInterpolation3d(
                    dim_in, n_classes, cfg.MRCNN.UPSAMPLE_RATIO)
        self._init_weights()
...
def mask_rcnn_fcn_head_v1up3convs(dim_in, roi_xform_func, spatial_scale):
    """v1up design: 3 * (conv 3x3), convT 2x2."""
    return mask_rcnn_fcn_head_v1upXconvs(
        dim_in, roi_xform_func, spatial_scale, 3
    )
...
def mask_rcnn_fcn_head_v1up(dim_in, roi_xform_func, spatial_scale):
    """v1up design: 1 * (conv 3x3), convT 2x2."""
    return mask_rcnn_fcn_head_v1upXconvs(
        dim_in, roi_xform_func, spatial_scale, 1
    )
...
class mask_rcnn_fcn_head_v1upXconvs(nn.Module):
    ...
    def __init__(self, dim_in, roi_xform_func, spatial_scale, num_convs):
        ...
        for i in range(num_convs):
            module_list.extend([
                nn.Conv3d(dim_in, dim_inner, 3, 1, padding=1*dilation, dilation=dilation),
        ...
        # upsample layer
        self.upconv = nn.ConvTranspose3d(dim_inner, dim_inner, 2, 2, 0)
