...
class fast_rcnn_outputs(nn.Module):
    def __init__(self, dim_in):
        ...
        if cfg.MODEL.CLS_AGNOSTIC_BBOX_REG:  # bg and fg
            self.bbox_pred = nn.Linear(dim_in, 6 * 2)
        else:
            self.bbox_pred = nn.Linear(dim_in, 6 * cfg.MODEL.NUM_CLASSES)
...
class roi_2mlp_head(nn.Module):
    ...
    def __init__(self, dim_in, roi_xform_func, spatial_scale):
        ...
        self.fc1 = nn.Linear(dim_in * roi_size**3, hidden_dim)
...
class roi_Xconv1fc_head(nn.Module):
    ...
    def __init__(self, dim_in, roi_xform_func, spatial_scale):
        ...
        for i in range(cfg.FAST_RCNN.NUM_STACKED_CONVS):
            module_list.extend([
                nn.Conv3d(dim_in, hidden_dim, 3, 1, 1),
        ...
        roi_size = cfg.FAST_RCNN.ROI_XFORM_RESOLUTION
        self.fc = nn.Linear(dim_in * roi_size * roi_size * roi_size, fc_dim)
