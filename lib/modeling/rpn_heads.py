...
from modeling.generate_anchors import generate_anchors, generate_anchors_3d
from modeling.generate_proposals_3d import GenerateProposalsOp_3d
from modeling.generate_proposal_labels_3d import GenerateProposalLabelsOp_3d
...
class single_scale_rpn_outputs(nn.Module):
    ...
    def __init__(self, dim_in, spatial_scale):
        ...
        self.RPN_GenerateProposals = GenerateProposalsOp_3d(anchors, spatial_scale)
        self.RPN_GenerateProposalLabels = GenerateProposalLabelsOp_3d()
    ...
    def forward(self, x, im_info, roidb=None):
        ...
        if not self.training or cfg.MODEL.FASTER_RCNN:
            ...
            if cfg.RPN.CLS_ACTIVATION == 'softmax':
                B, C, S, H, W = rpn_cls_logits.size()
                rpn_cls_prob = F.softmax(
                    rpn_cls_logits.view(B, 2, C // 2, S, H, W), dim=1)
                rpn_cls_prob = rpn_cls_prob[:, 1].squeeze(dim=1)
            else:
                rpn_cls_prob = F.sigmoid(rpn_cls_logits)

            rpn_rois, rpn_rois_prob, scores_keep_idx = self.RPN_GenerateProposals(
                rpn_cls_prob, rpn_bbox_pred, im_info)

            if not self.training and cfg.PRM_ON:
                return_dict['scores_keep_idx'] = scores_keep_idx
                return_dict['class_response_maps'] = rpn_cls_prob

            return_dict['rpn_rois'] = rpn_rois  # the coordinates of proposals
            return_dict['rpn_roi_probs'] = rpn_rois_prob # the score of proposals
...
def single_scale_rpn_losses(
    ...
    s, h, w = rpn_cls_logits.shape[2:]
    rpn_labels_int32 = rpn_labels_int32_wide[:, :, :s, :h, :w]   # -1 means ignore
    s, h, w = rpn_bbox_pred.shape[2:]
    rpn_bbox_targets = rpn_bbox_targets_wide[:, :, :s, :h, :w]
    rpn_bbox_inside_weights = rpn_bbox_inside_weights_wide[:, :, :s, :h, :w]
    rpn_bbox_outside_weights = rpn_bbox_outside_weights_wide[:, :, :s, :h, :w]

    if cfg.RPN.CLS_ACTIVATION == 'softmax':
        B, C, S, H, W = rpn_cls_logits.size()
        rpn_cls_logits = rpn_cls_logits.view(
            B, 2, C // 2, S, H, W).permute(0, 2, 3, 4, 5, 1).contiguous().view(-1, 2)
