...
from utils.segms import mask_to_bbox_3d
...
class NucleiDataset(object):
    """A class representing my nuclei dataset."""
    ...
    def get_roidb(
        ...
        for entry in roidb:
            self._prep_roidb_entry(entry, phase) # time-consuming
        if gt:
    ...
    def _prep_roidb_entry(self, entry, phase):
        """Adds empty metadata fields to an roidb entry."""
        # Reference back to the parent dataset
        entry['dataset'] = self
        # Make file_name an abs path
        if entry['file_name'][-3:] != 'tif':
            im_path = os.path.join(
                self.image_directory, entry['file_name'] + '.tif'
            )
        else:
            # if the directory is absolute
            im_path = entry['file_name']
        ...
        if phase == 'train':
            # for nuclei dataset the mask labels include mask tif and mask val
            entry['mask'] = io.imread(os.path.join(self.label_directory, 'mask', entry['file_name'] + '.tif')).astype(
               np.uint8)
            entry['need_crop'] = cfg.TRAIN.NEED_CROP

    def _load_ann_objs(self, file_name):
        bbox_file_path = os.path.join(self.label_directory, 'bbox', file_name+'.txt')
        mask = io.imread(os.path.join(self.label_directory, 'mask', file_name+'.tif'))
        fid_ann = open(bbox_file_path, 'r')
        ...
        for a in ann:
            ...
            x1 = np.minimum(width - 1,  int(float(part_a[1])))
            y1 = np.minimum(height - 1, int(float(part_a[2])))
            z1 = np.minimum(slices - 1, int(float(part_a[3])))
            w = int(float(part_a[4]))
            h = int(float(part_a[5]))
            s = int(float(part_a[6]))
            marker = int(part_a[7])
            obj = {}
            obj['bbox'] = [x1, y1, z1, w, h, s]
            obj['category_id'] = self.category_to_id_map['nuclei']
            crop_mask = mask[z1: z1+s, y1: y1+h, x1: x1+w].copy()
            obj['volume'] = np.sum(crop_mask == marker)
            if np.sum(crop_mask==marker) == 0:
                print(file_name)
            crop_mask = crop_mask==marker
            crop_mask = crop_mask.astype(np.uint8)
            obj['segmentation'] = binary_mask_to_rle(crop_mask)

            condition1 = ((x1 == 0)  or (x1+w-1>=width)) and (w < 25)
            condition2 = ((y1 == 0) or (y1+h-1>=height)) and (h < 25)
            condition3 = valid or (a!=ann[-1])
