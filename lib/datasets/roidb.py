...
from .soma_dataset import SomaDataset
...
def combined_roidb_for_training(dataset_names, proposal_files):
    ...
    def get_roidb(dataset_name, proposal_file):
        if cfg.TRAIN.DATASETS == ('soma_det_seg_train',):
            ds = SomaDataset(dataset_name)
        elif cfg.TRAIN.DATASETS == ('nuclei_det_seg_train',):
            ds = NucleiDataset(dataset_name)
        roidb = ds.get_roidb(
            gt=True,
...
def _compute_targets(entry):
    ...
    # Targets has format (class, tx, ty, tz, tw, th, ts)
    targets = np.zeros((rois.shape[0], 7), dtype=np.float32)
    if len(gt_inds) == 0:
    ...
    ex_gt_overlaps = box_utils_3d.bbox_overlaps_3d(
    ...
    targets[ex_inds, 1:] = box_utils_3d.bbox_transform_inv_3d(
