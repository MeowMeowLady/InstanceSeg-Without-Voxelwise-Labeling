...
def get_rpn_blob_names(is_training=True):
    ...
    blob_names = ['im_info']
    if is_training:
        # gt boxes: (batch_idx, x1, y1, z1, x2, y2, z2, cls)
        blob_names += ['roidb']
...
def add_rpn_blobs(blobs, im_scales, roidb):
    ...
    for im_i, entry in enumerate(roidb):
        ...
        im_slices = np.around(entry['slices']*scale)
        ...
        dc_inds = np.where(entry['is_crowd'])[0]
        dc_rois = entry['boxes'][dc_inds, :] * scale
        ...
        im_info = np.array([[im_slices, im_height, im_width, scale]], dtype=np.float32)
        ...
        if cfg.FPN.FPN_ON and cfg.FPN.MULTILEVEL_RPN:
            ...
            rpn_blobs = _get_rpn_blobs(
                im_slices, im_height, im_width, foas, all_anchors, gt_rois, dc_rois
            )
            ...
            rpn_blobs = _get_rpn_blobs(
                im_slices, im_height, im_width, [foa], all_anchors, gt_rois, dc_rois
            )
    ...
    valid_keys = [
        'has_visible_keypoints', 'boxes', 'segms', 'seg_volumes', 'gt_classes',
...
def _get_rpn_blobs(im_slices, im_height, im_width, foas, all_anchors, gt_boxes, dc_boxes):
    ...
    if straddle_thresh >= 0:
        ...
        inds_inside = np.where(
            ...
            (all_anchors[:, 5] < im_slices + straddle_thresh)
        )[0]
    ...
    if len(gt_boxes) > 0:
        ...
        anchor_by_gt_overlap = box_utils_3d.bbox_overlaps_3d(anchors, gt_boxes)
        # Map from anchor to gt box that has highest overlap. find which gt box has max ovlp with one anchor
        ...
        # dont care boxes
        if dc_boxes.size:
            anchor_by_dc_overlap = box_utils_3d.bbox_overlaps_3d(anchors, dc_boxes)
            anchor_to_dc_argmax = anchor_by_dc_overlap.argmax(axis=1)
            anchor_to_dc_max = anchor_by_dc_overlap[np.arange(num_inside),
                                                    anchor_to_dc_argmax]
        else:
            anchor_to_dc_max = np.zeros(anchor_to_gt_max.shape)
    ...
    bg_inds = np.where((anchor_to_gt_max < cfg.TRAIN.RPN_NEGATIVE_OVERLAP) & \
                       (anchor_to_dc_max < cfg.TRAIN.RPN_NEGATIVE_OVERLAP))[0]  # dont care boxes
    ...
    for foa in foas:
        ...
        # labels output with shape (1, A, slices, height, width)
        _labels = _labels.reshape((1, S, H, W, A)).transpose(0, 4, 1, 2, 3)
        # bbox_targets output with shape (1, 6 * A, slices, height, width)
        _bbox_targets = _bbox_targets.reshape(
            (1, S, H, W, A * 6)).transpose(0, 4, 1, 2, 3)
        # bbox_inside_weights output with shape (1, 6 * A, slices, height, width)
        _bbox_inside_weights = _bbox_inside_weights.reshape(
            (1, S, H, W, A * 6)).transpose(0, 4, 1, 2, 3)
        # bbox_outside_weights output with shape (1, 6 * A, slices, height, width)
