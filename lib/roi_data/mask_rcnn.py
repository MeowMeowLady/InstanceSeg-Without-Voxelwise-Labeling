...
def add_mask_rcnn_blobs(blobs, sampled_boxes, roidb, im_scale, batch_idx):
    ...
    if cfg.MRCNN.ANNO_TYPE == 'spot':
        spots_gt_inds = np.where((roidb['gt_classes'] > 0) &
                                 (roidb['is_crowd'] == 0))[0]
        segms_gt = [roidb['segms'][i] for i in spots_gt_inds]
        boxes_gt = segm_utils.spots_to_boxes(segms_gt)
    else:
        rle_gt_inds = np.where((roidb['gt_classes'] > 0) &
                                 (roidb['is_crowd'] == 0))[0]
        segms_gt = [roidb['segms'][i] for i in rle_gt_inds]
        boxes_gt = roidb['boxes'][rle_gt_inds, :]
    ...
    if fg_inds.shape[0] > 0:
        ...
        overlaps_bbfg_bbspots = box_utils_3d.bbox_overlaps_3d(
            rois_fg.astype(np.float32, copy=False),
            boxes_gt.astype(np.float32, copy=False))
        # Map from each fg rois to the index of the mask with highest overlap
        # (measured by bbox overlap)
        fg_segms_inds = np.argmax(overlaps_bbfg_bbspots, axis=1)
        ...
        for i in range(rois_fg.shape[0]):
            ...
            roi_fg = rois_fg[i]
            box_gt = boxes_gt[fg_segms_ind]
            # Rasterize the portion of the polygon mask within the given fg roi
            # to an M x M binary image
            if cfg.MRCNN.ANNO_TYPE == 'spot':
                mask = segm_utils.spot_to_mask_wrt_box(segm_gt, roi_fg, M)
            else:
                mask = segm_utils.rle_to_mask_wrt_box(segm_gt, box_gt, roi_fg, M)
    ...
    # Scale rois_fg and format as (batch_idx, x1, y1, z1, x2, y2, z2)
    rois_fg *= im_scale
...
def _expand_to_class_specific_mask_targets(masks, mask_class_labels):
    """Expand masks from shape (#masks, M ** 3) to (#masks, #classes * M ** 3)
    ...
    mask_targets = -blob_utils.ones(
        (masks.shape[0], cfg.MODEL.NUM_CLASSES * M**3), int32=True)
