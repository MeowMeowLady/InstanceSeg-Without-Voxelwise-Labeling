...
def get_fast_rcnn_blob_names(is_training=True):
    ...
    # (batch_idx, x1, y1, z1, x2, y2, z2) specifying an image batch index and a
    ...
    if is_training:
        # bbox_targets blob: R bounding-box regression targets with 6
        # targets per class
        blob_names += ['bbox_targets']
        # bbox_inside_weights blob: At most 6 targets per roi are active
    ...
    if is_training and cfg.MODEL.MASK_ON:
        ...
        # Shape is (#masks, 6) in format (batch_idx, x1, y1, z1, x2, y2, z2).
        ...
        # 'mask_rois'. Shape is (#fg, M * M * M) where M is the ground truth
        # mask size.
        # if cfg.MRCNN.CLS_SPECIFIC_MASK: Shape is (#masks, #classes * M ** 3)
...
def _sample_rois(roidb, im_scale, batch_idx):
    ...
    # Scale rois and format as (batch_idx, x1, y1, z1, x2, y2, z2)
...
def _compute_targets(ex_rois, gt_rois, labels):
    ...
    targets = box_utils_3d.bbox_transform_inv_3d(ex_rois, gt_rois,
...
def _expand_bbox_targets(bbox_target_data):
    ...
    Returns:
        bbox_target_data (ndarray): N x 6K blob of regression targets
        bbox_inside_weights (ndarray): N x 6K blob of loss weights
    """
