...
class PeakStimulation(Function):
    ...
    def forward(ctx, input, return_aggregation, win_size, peak_filter):
        ...
        offset = (win_size - 1) // 2
        padding = torch.nn.ConstantPad3d(offset, float('-inf'))
        padded_maps = padding(input)
        batch_size, num_channels, s, h, w = padded_maps.size()
        element_map = torch.arange(0, s * h * w).long().view(1, 1, s, h, w)[:, :, offset: -offset, offset: -offset, offset: -offset]
    ...
    def backward(ctx, grad_peak_list, grad_output):
        ...
        batch_size, num_channels, _, _, _ = input.size()
        grad_input = peak_map * grad_output.view(batch_size, num_channels, 1, 1, 1)
        return (grad_input,) + (None,) * ctx.num_flags


def peak_stimulation_3d(input, return_aggregation=True, win_size=3, peak_filter=None):
