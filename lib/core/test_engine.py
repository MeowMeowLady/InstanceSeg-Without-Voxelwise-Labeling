...
from datasets.soma_dataset import SomaDataset
...
import utils.my_subprocess as subprocess_utils
...
def run_inference(args, multi_gpu_testing=False, gpu_id=0):
...
def test_net_on_dataset(
    ...
    if cfg.TEST.DATASETS == ('soma_det_seg_train',):
        dataset = SomaDataset(dataset_name)
    elif cfg.TEST.DATASETS == ('nuclei_det_seg_train',):
        dataset = NucleiDataset(dataset_name)
    test_timer = Timer()
    test_timer.tic()
    if multi_gpu:
    ...
    results = {'all_boxes': all_boxes, 'all_segms': all_segms}
    return results
...
def get_roidb_and_dataset(dataset_name, proposal_file, ind_range):
    ...
    if cfg.TEST.PRECOMPUTED_PROPOSALS:
        ...
        roidb = dataset.get_roidb(phase='test')
...
def empty_results(num_classes, num_images):
    ...
    Box detections are collected into:
      all_boxes[cls][image] = N x 7 array with columns (x1, y1, z1, x2, y2, z2, score)
